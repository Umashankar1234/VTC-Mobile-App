...
export const InteriorsScreen = ({ route, navigation }) => {
  ...
  const [interiorAmenities, setInteriorAmenities] = React.useState([]);
      ...
      postMethod("get-amenities", obj).then((res) => {
        if (res.length > 0 && componentMounted) {
          if (res[0].response.status === "success") {
            setRefresh(!refresh);
            setInteriorAmenities(res[0].response.data.interior);
          }
        }
      });
    }
  ...
  const onActiveCheckedChange = (isChecked, index) => {
    let newArr = interiorAmenities.map((item, i) => {
      if (index === i) {
