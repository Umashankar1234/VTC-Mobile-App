...
export const AppliancesScreen = ({ route, navigation }) => {
  ...
  const [appliancesAmenities, setAppliancesAmenities] = React.useState([]);
  ...
  const onActiveCheckedChange = (isChecked, index) => {
    let newArr = appliancesAmenities.map((item, i) => {
      if (index === i) {
