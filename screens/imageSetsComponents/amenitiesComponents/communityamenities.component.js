...
export const CommunityScreen = ({ route, navigation }) => {
  ...
  const [communityAmenities, setCommunityAmenities] = React.useState([]);
      ...
      postMethod("get-amenities", obj).then((res) => {
        if (res.length > 0 && componentMounted) {
          if (res[0].response.status === "success") {
            setRefresh(false);
            setCommunityAmenities(res[0].response.data.community);
          }
        }
      });
    }
  ...
  const onActiveCheckedChange = (isChecked, index) => {
    let newArr = communityAmenities.map((item, i) => {
      if (index === i) {
              ...
              {communityAmenities && communityAmenities.length > 0 ? (
                communityAmenities.map((am, i) => (
                  ...
                  >
                    <Toggle
                      style={styles.toggle}
                      checked={am.countamenity === 1 ? true : false}
                      onChange={(nextValue) =>
                        onActiveCheckedChange(nextValue, i)
                      }
                    >
                      {am.amenityname}
                    </Toggle>
                    {userData.agent_id === am.agentid && (
                      <Pressable
                        // style={styles.modalPressableClose}
                        onPress={() => removeAmenitiy(am)}
                      >
                        <Icon
                          name="trash-outline"
                          style={styles.closeIcon}
