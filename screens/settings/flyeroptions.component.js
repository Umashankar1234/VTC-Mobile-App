...
export const FlyerOptionsScreen = ({ navigation }) => {
  ...
  React.useEffect(() => {
    ...
    postMethod("agent-get-flyer-option", obj).then((res) => {
      if (res.length > 0 && componentMounted) {
        if (res[0].response.status === "success") {
          res[0].response.data.flyer_option.is_flyer_agent_company_photo === 0
            ? setAllowCompanyLogo(1)
            : setAllowCompanyLogo(0);
          res[0].response.data.flyer_option.is_flyer_agent_photo === 0
  ...
  const updateFlyerOption = () => {
    setLoading(true);
    let obj = {
      agent_id: userData.agent_id,
      is_flyer_agent_company_photo: allowCompanyLogo === 0 ? 1 : 0,
      is_flyer_agent_photo: allowAgentPhoto === 0 ? 1 : 0,
    };

    postMethod("agent-update-flyer-option", obj).then((res) => {
      if (res.length > 0) {
      ...
      <Layout style={styles.container}>
        <ScrollView>
          <View style={styles.headingWrap}>
            <FontAwesomeIcon icon={faBookOpen} size={20} color={"#adadad"} />
          ...
          <View styles={styles.wrapTab}>
            <Text category="s1" style={{ padding: 10 }}>
              ...
              Flyers That System Compiles For Your Tours.
            </Text>
